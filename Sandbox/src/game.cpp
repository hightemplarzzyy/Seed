...
public:
	...
	virtual void OnAttach() override
	{
		...
		m_Mesh.reset(new Seed::Mesh("assets/models/m1911/m1911.fbx"));
		m_MeshMaterial.reset(new Seed::MaterialInstance(m_Mesh->GetMaterial()));

		m_QuadShader = Seed::Shader::Create("assets/shaders/quad.glsl");
		m_HDRShader = Seed::Shader::Create("assets/shaders/hdr.glsl");
		m_SphereMesh.reset(new Seed::Mesh("assets/models/Sphere1m.fbx"));
		m_PlaneMesh.reset(new Seed::Mesh("assets/models/Plane1m.obj"));

		m_GridShader = Seed::Shader::Create("assets/shaders/Grid.glsl");
		m_GridMaterial = Seed::MaterialInstance::Create(Seed::Material::Create(m_GridShader));
		m_GridMaterial->Set("u_Scale", m_GridScale);
		m_GridMaterial->Set("u_Res", m_GridSize);

		// Editor
		m_CheckerboardTex.reset(Seed::Texture2D::Create("assets/editor/Checkerboard.tga"));

		// Environment
		m_EnvironmentCubeMap.reset(Seed::TextureCube::Create("assets/textures/environments/Arches_E_PineTree_Radiance.tga"));
		m_EnvironmentIrradiance.reset(Seed::TextureCube::Create("assets/textures/environments/Arches_E_PineTree_Irradiance.tga"));
		m_BRDFLUT.reset(Seed::Texture2D::Create("assets/textures/BRDF_LUT.tga"));

		m_Framebuffer.reset(Seed::Framebuffer::Create(1280, 720, Seed::FramebufferFormat::RGBA16F));
		m_FinalPresentBuffer.reset(Seed::Framebuffer::Create(1280, 720, Seed::FramebufferFormat::RGBA8));

		float x = -4.0f;
		float roughness = 0.0f;
		for (int i = 0; i < 8; i++)
		{
			Seed::Ref<Seed::MaterialInstance> mi(new Seed::MaterialInstance(m_SphereMesh->GetMaterial()));
		...
		m_VertexBuffer.reset(Seed::VertexBuffer::Create());
		m_VertexBuffer->SetData(data, 4 * sizeof(QuadVertex));

		uint32_t* indices = new uint32_t[6]{ 0, 1, 2, 2, 3, 0, };
		m_IndexBuffer.reset(Seed::IndexBuffer::Create());
	...
	virtual void OnUpdate(Seed::TimeStep ts) override
	{
		...
		Seed::UniformBufferDeclaration<sizeof(mat4), 1> quadShaderUB;
	...
	virtual void OnImGuiRender() override
	{
		...
		ImGui::Separator();
		{
			...
			{
				std::string filename = Seed::Application::Get().OpenFile("");
				if (filename != "")
				{
					m_Mesh.reset(new Seed::Mesh(filename));
					m_MeshMaterial.reset(new Seed::MaterialInstance(m_Mesh->GetMaterial()));
				}
			}
		}
			...
			if (ImGui::CollapsingHeader("Albedo", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				if (ImGui::IsItemHovered())
				{
					...
					if (ImGui::IsItemClicked())
					{
						std::string filename = Seed::Application::Get().OpenFile("");
						if (filename != "")
							m_AlbedoInput.TextureMap.reset(Seed::Texture2D::Create(filename, m_AlbedoInput.SRGB));
					}
				}
				ImGui::SameLine();
				ImGui::BeginGroup();
				...
				{
					if (m_AlbedoInput.TextureMap)
						m_AlbedoInput.TextureMap.reset(Seed::Texture2D::Create(m_AlbedoInput.TextureMap->GetPath(), m_AlbedoInput.SRGB));
				}
				ImGui::EndGroup();
				ImGui::SameLine();
			...
			if (ImGui::CollapsingHeader("Normals", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				if (ImGui::IsItemHovered())
				{
					...
					if (ImGui::IsItemClicked())
					{
						std::string filename = Seed::Application::Get().OpenFile("");
						if (filename != "")
							m_NormalInput.TextureMap.reset(Seed::Texture2D::Create(filename));
					}
				}
				ImGui::SameLine();
			...
			if (ImGui::CollapsingHeader("Metalness", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				if (ImGui::IsItemHovered())
				{
					...
					if (ImGui::IsItemClicked())
					{
						std::string filename = Seed::Application::Get().OpenFile("");
						if (filename != "")
							m_MetalnessInput.TextureMap.reset(Seed::Texture2D::Create(filename));
					}
				}
				ImGui::SameLine();
			...
			if (ImGui::CollapsingHeader("Roughness", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				if (ImGui::IsItemHovered())
				{
					...
					if (ImGui::IsItemClicked())
					{
						std::string filename = Seed::Application::Get().OpenFile("");
						if (filename != "")
							m_RoughnessInput.TextureMap.reset(Seed::Texture2D::Create(filename));
					}
				}
				ImGui::SameLine();
		...
		{
			auto& shaders = Seed::Shader::s_AllShaders;
		...
		auto [wx, wy] = Seed::Application::Get().GetWindow().GetWindowPos();
		posX -= wx;
		posY -= wy;
		SEED_INFO("{0}, {1}", posX, posY);*/
	...
	virtual void OnEvent(Seed::Event& event) override
	{
	}

private:
	Seed::Ref<Seed::Shader> m_QuadShader;
	Seed::Ref<Seed::Shader> m_HDRShader;
	Seed::Ref<Seed::Shader> m_GridShader;
	Seed::Ref<Seed::Mesh> m_Mesh;
	Seed::Ref<Seed::Mesh> m_SphereMesh, m_PlaneMesh;
	Seed::Ref<Seed::Texture2D> m_BRDFLUT;

	Seed::Ref<Seed::MaterialInstance> m_MeshMaterial;
	Seed::Ref<Seed::MaterialInstance> m_GridMaterial;
	std::vector<Seed::Ref<Seed::MaterialInstance>> m_MetalSphereMaterialInstances;
	std::vector<Seed::Ref<Seed::MaterialInstance>> m_DielectricSphereMaterialInstances;
	...
	struct AlbedoInput
	{
		...
		Seed::Ref<Seed::Texture2D> TextureMap;
		bool SRGB = true;
	...
	Seed::Ref<Seed::Framebuffer> m_Framebuffer, m_FinalPresentBuffer;

	Seed::Ref<Seed::VertexBuffer> m_VertexBuffer;
	Seed::Ref<Seed::IndexBuffer> m_IndexBuffer;
	Seed::Ref<Seed::TextureCube> m_EnvironmentCubeMap, m_EnvironmentIrradiance;

	Seed::Camera m_Camera;
	...
	// Editor resources
	Seed::Ref<Seed::Texture2D> m_CheckerboardTex;
};
